...
class HashGeneratorApp(QWidget):
    ...
    def on_submit(self):
        ...
        current_weather = accessWeather()
        temp, humidity, speed, city, country, weather = current_weather
    ...
    def get_passwords(self):
        ...
        return self.user_password, self.user_hashed_password, self.quadrant_data,
...
user_password, hashed_password, quadrant_data = window.get_passwords()
...
def store_password_entry():
    ...
    time = current_time
    # Store the actual weather components used in calculation, not just the final value
    temp, humidity, speed, city, country, weather = current_weather  # Unpack the tuple
    ...
    entry = {
        ...
        "Time": time,
        "Weather Components": {  # Store all weather components
            "temp": temp,
            "humidity": humidity,
            "speed": speed,
            "city": city,
            "country": country,
            "calculated_value": weather
        },
        "Weather": weather,  # Keep this for backward compatibility
...
def recreate_hash(username):
    """Retrieve stored data and recreate the hash using identical string formatting"""
    ...
    try:
        # Extract all components exactly as stored
        original_password = user_data["original_password"]
        stored_hash = user_data["hashed_password"]
        quadrant_data = user_data["Quadrant Data"]
        time = user_data["Time"]
        weather = user_data["Weather"]  # This should be the exact calculated value
        
        # DEBUG: Print all components
        print("\nDEBUG - Components for recreation:")
        print(f"Password: '{original_password}'")
        print("Quadrant Data:", quadrant_data)
        print(f"Weather: {weather} ({type(weather)})")
        print(f"Time: '{time}'")
        
        # EXACTLY replicate the original hashing process
        quad_str = ''.join(
            f"{q['count']}_{q['avg'][0]:.2f}_{q['avg'][1]:.2f}" 
            for q in quadrant_data.values()
        )
        horse = str(weather)
        combined = original_password + quad_str + horse + time
        
        # DEBUG: Show the concatenated string
        print("\nDEBUG - Combined string before hashing:")
        print(f"'{combined}'")
        print(f"Length: {len(combined)}")
        print(f"Bytes: {combined.encode('utf-8')}")
        ...
        print(f"Error during hash recreation: {e}")
        return None
if __name__ == "__main__":
    #username = input("Enter a username: ")
    store_password_entry()
    result = recreate_hash(username)
    print(result)
